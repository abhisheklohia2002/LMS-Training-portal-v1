...
export function DashboardPage() {
  ...
  if ([users, courses, assignments, certs, attempts].some((q) => q.isLoading))
    return <Skeleton active />;
  const pending =
    assignments.data?.filter((a) => a.status !== "completed").length ?? 0;
  const pass =
    attempts.data?.filter((a) => a.result_status === "passed").length ?? 0;
  const passRate = attempts.data?.length
    ? Math.round((pass / attempts.data.length) * 100)
    : 0;
  return (
    <>
      <PageHeader
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard
          ...
          value={courses.data?.filter((c) => c.is_active).length ?? 0}
          icon={<BookOutlined />}
        />
        <MetricCard
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
        <Card title="Training completion" className="page-card lg:col-span-2">
          <div className="space-y-4">
            <Progress
              percent={Math.round(
                ((assignments.data?.filter((a) => a.status === "completed")
                  .length ?? 0) /
                  (assignments.data?.length || 1)) *
                  100,
              )}
            />
            <Progress percent={passRate} status="active" />
          </div>
        </Card>
        <Card title="Recent activity" className="page-card">
          <Timeline
            items={[
              { children: "Course assigned to David" },
              { children: "Password quiz passed" },
              { children: "Certificate issued" },
            ]}
          />
        </Card>
      </div>
